    ...
    >
      <Container className="flex flex-col gap-10 md:gap-12">
        <header className="flex flex-col gap-4 md:gap-5">
          ...
          <div className="grid gap-4 md:grid-cols-12 md:items-end md:gap-6">
            ...
            <p
              data-process-support
              className="r6-body max-w-sm text-muted md:col-span-4 md:col-start-9"
            >
        ...
        <div className="relative">
          <div
            data-process-line
            className="absolute left-0 top-5 h-px w-full bg-line origin-left md:top-1/2 md:-translate-y-1/2"
            aria-hidden
          />
          <ol className="relative flex flex-col gap-6 md:grid md:grid-cols-5 md:gap-6 md:pt-10">
            {processSteps.map((step, index) => (
              <li
                key={step.index}
                data-process-item
                className="relative flex flex-col gap-2 pt-6 md:pt-0"
              >
                <span className="r6-overline text-acid" aria-hidden>
                  {step.index}
                </span>
                <h3 className="r6-h3 uppercase">{step.title}</h3>
                <p className="r6-small text-muted">{step.description}</p>
                {index < processSteps.length - 1 ? (
                  <span className="hidden h-px w-6 bg-line md:block" aria-hidden />
                ) : null}
              </li>
            ))}
          </ol>
      ...
      <Container className="mt-10 md:mt-12">
        <div className="flex flex-col gap-4 border-t border-line pt-5 md:flex-row md:items-center md:justify-between md:gap-6">
          <p className="r6-body-lg max-w-lg">
            <span className="block uppercase">Grandes experiências</span>
            <span className="block uppercase">começam com uma boa direção.</span>
          </p>
          <Link
            href="/sobre"
            className="r6-link-underline inline-flex min-h-9 items-center gap-2 text-xs font-medium tracking-[0.12em] uppercase text-bone"
          >
            Conheça a R6
