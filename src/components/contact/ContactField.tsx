...
>(({ label, options, ...props }, ref) => (
  <div className="relative flex flex-col gap-3">
    <h3 className="r6-h3 uppercase">{label}</h3>
    <select ref={ref} className="w-full appearance-none rounded-full border border-line bg-void px-4 py-3 text-bone focus:border-acid focus:outline-none transition-colors duration-300" {...props}>
      {options.map((opt) => (
...
function inputCn(error?: string, extra = "rounded-full") {
  return `w-full border ${error ? "border-red-400" : "border-line"} bg-void ${extra} px-4 py-3 text-bone placeholder:text-muted focus:border-acid focus:outline-none transition-colors duration-300`;
}